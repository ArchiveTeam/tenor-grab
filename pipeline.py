...
LUA_SHA1 = get_hash(os.path.join(CWD, 'tenor.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'post':
                wget_args.extend(['--warc-header', 'tenor-post: '+item_value])
                wget_args.append('https://tenor.com/view/gif-{}'.format(item_value))
            #elif item_type == 'media':
            #    pass
            elif item_type == 'search':
                locale, term = item_value.split(':', 1)
                if locale != 'en':
                    raise ValueError('Only search:en: items are supported for now.')
                wget_args.extend(['--warc-header', 'tenor-search: '+item_value])
                term_path = quote(re.sub(r'\s+', '-', unquote(term)))
                wget_args.append('https://tenor.com/search/{}-gifs'.format(term_path))
            elif item_type == 'suggest':
                locale, prefix = item_value.split(':', 1)
                wget_args.extend(['--warc-header', 'tenor-suggest: '+item_value])
                prefix = quote(unquote(prefix))
                wget_args.append('https://tenor.com/?suggest={}&locale={}'.format(prefix, locale))
            elif item_type == 'user':
                wget_args.extend(['--warc-header', 'tenor-user: '+item_value])
                wget_args.append('https://tenor.com/users/{}'.format(
                    quote(unquote(item_value))))
            else:
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/2/24/Tenor-icon.png" height="50px" title=""/>
        <h2>Tenor <span class="links"><a href="https://tenor.com/">Website</a> &middot; <a href="https://tracker.archiveteam.org/tenor/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/Tenor">Wiki</a></span></h2>
        <p>Archiving Tenor API and web pages.</p>
    ''',
    utc_deadline=datetime.datetime(2026, 6, 30, 0, 0, 0)
)
